...
export const useDynamicFilters = () => {
    ...
    const groupLabels: Record<string, string> = {
        ...
        'digital-perspectives': 'Digital Perspective',
        'digital-streams': 'Digital Stream',
        'digital-sectors': 'Digital Sector',
        'content-format': 'Format',
        'popularity-tags': 'Popularity Tag',
        ...
        try {
            ...
            for (const group of groups) {
                ...
                if (isHierarchical) {
                    ...
                    const hierarchicalData = await categoryService.getCategoriesGroupedByFilterGroup(group);
                    
                    const parentOptions: FilterOption[] = hierarchicalData.map(parent => ({
                        id: parent.id,
                        label: parent.name,
                        value: parent.name
                    }));
                    ...
                    // Fetch flat data
                    const flatData = await categoryService.getCategoriesByFilterGroup(group);
    ...
    const getChildOptions = (groupName: string, parentId: string): FilterOption[] => {
