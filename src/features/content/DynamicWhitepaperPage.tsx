...
const DropCap = ({ children }: { children: string }) => {
  if (!children || typeof children !== 'string') {
    return <p className="text-lg leading-relaxed text-gray-900 font-serif mb-6">{children}</p>;
  }
...
const DynamicWhitepaperPage: React.FC = () => {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()

  const [whitepaper, setWhitepaper] = useState<any | null>(null)
  const [whitepaperContent, setWhitepaperContent] = useState<any | null>(null)
  ...
  useEffect(() => {
    const fetchWhitepaper = async () => {
      try {
        ...
        const whitepaperData = response.data?.find(item => item.id === id)
        if (whitepaperData) {
          setWhitepaper(whitepaperData)
          
          // Parse the JSON content
          if (whitepaperData.content) {
            try {
              const parsedContent = JSON.parse(whitepaperData.content);
              setWhitepaperContent(parsedContent);
            } catch (parseError) {
              console.error('Error parsing whitepaper content:', parseError);
              setWhitepaperContent(null);
            }
          }
        } else {
          // Fallback to first whitepaper if specific ID not found
          if (response.data && response.data.length > 0) {
            const fallbackPaper = response.data[0];
            setWhitepaper(fallbackPaper);
            
            if (fallbackPaper.content) {
              try {
                const parsedContent = JSON.parse(fallbackPaper.content);
                setWhitepaperContent(parsedContent);
              } catch (parseError) {
                console.error('Error parsing fallback content:', parseError);
                setWhitepaperContent(null);
              }
            }
          }
  ...
  const HeroSection = () => {
    return (
      <section
        ...
        style={{
          backgroundImage: `url(${whitepaperContent?.hero?.heroImageUrl || whitepaper?.heroImage || heroImg})`,
      ...
      >
        ...
        <div className="relative z-10 text-center text-white px-6">
          <p className="text-lg md:text-xl mb-4 tracking-wide opacity-90 font-serif">
            {whitepaperContent?.hero?.subtitle || 'Digital Transformation Management Book | Volume 0'}
          </p>
          <p className="text-2xl md:text-3xl mb-6 font-bold tracking-wider">
            {whitepaperContent?.hero?.volumeLabel || 'D1'}
          </p>
          <h1 className="text-5xl md:text-7xl font-serif">
            {whitepaperContent?.hero?.title || whitepaper?.title || 'Digital Economy 4.0'}
          </h1>
        </div>
      </section>
    );
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-gray-600">Loading whitepaper...</div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <ContentSection>
        {(() => {
          const hookText = whitepaperContent?.hookText || whitepaper?.excerpt || whitepaper?.summary || 'The economy has transformed; organizations must ask if they have too. Those that don\'t evolve risk becoming spectators in a system already moving past them. But recognizing the shift is only the first step. The real question is this: How can organizations redesign themselves to lead in the new economy?';
          
          // Extract plain text from HTML for DropCap
          const plainText = hookText.replace(/<[^>]*>/g, '');
          
          return (
            <>
              <DropCap>{plainText}</DropCap>
              {whitepaperContent?.hookText ? (
                <div dangerouslySetInnerHTML={{ __html: whitepaperContent.hookText.replace(/^<p[^>]*>/, '').replace(/<\/p>$/, '') }} />
              ) : (
                <p className="text-base leading-relaxed text-gray-700 font-serif">
                  {plainText.substring(1)} {/* Remove first character since it's in DropCap */}
                </p>
              )}
            </>
          );
      ...
      {/* Foreword */}
      <ParallaxSection 
        image={whitepaperContent?.foreword?.heroImageUrl || forewordImg} 
        title={whitepaperContent?.foreword?.title || "Foreword"} 
        id="foreword" 
      />
      ...
      <ContentSection>
        <div className="flex items-start gap-6 mb-8">
          <img
            src={whitepaperContent?.foreword?.author?.imageUrl || whitepaper?.author?.avatar || authorImg}
            alt={whitepaperContent?.foreword?.author?.name || whitepaper?.author?.name || 'Dr. Stephane Niango'}
            className="w-20 h-20 rounded-full object-cover flex-shrink-0"
          />
          <div>
            <h3 className="text-lg font-bold text-gray-900">
              {whitepaperContent?.foreword?.author?.name || whitepaper?.author?.name || 'Dr. Stephane Niango'}
            </h3>
            <p className="text-sm text-gray-600 italic">
              {whitepaperContent?.foreword?.author?.title || whitepaper?.author?.title || 'CEO | Chief Architect at DigitalQatalyst (Bio)'}
            </p>
          </div>
        </div>

        <div dangerouslySetInnerHTML={{ __html: whitepaperContent?.foreword?.content || `
      ...
      {/* Executive Summary */}
      <ParallaxSection 
        image={whitepaperContent?.executiveSummary?.heroImageUrl || executiveImg} 
        title={whitepaperContent?.executiveSummary?.title || "Executive Summary"} 
        id="executive-summary" 
      />

      <ContentSection>
        {whitepaperContent?.executiveSummary?.content ? (
          (() => {
            const content = whitepaperContent.executiveSummary.content;
            const plainText = content.replace(/<[^>]*>/g, '');
            
            return (
              <>
                <DropCap>{plainText}</DropCap>
                <div dangerouslySetInnerHTML={{ __html: content.replace(/^<p[^>]*>/, '').replace(/<\/p>$/, '') }} />
              </>
            );
          })()
        ) : (
      ...
      {/* Dynamic Chapters */}
      {whitepaperContent?.chapters?.map((chapter: any, index: number) => (
        <React.Fragment key={chapter.id || index}>
          <ParallaxSection 
            image={chapter.heroImageUrl || chapter1Img} 
      ...
      {/* Fallback Chapter 1 if no dynamic chapters */}
      {!whitepaperContent?.chapters || whitepaperContent.chapters.length === 0 ? (
        <>
      ...
      <ParallaxSection 
        ...
        title={whitepaperContent?.conclusion?.title || "Conclusion"} 
        id="conclusion" 
      />

      <ContentSection>
        <div dangerouslySetInnerHTML={{ __html: whitepaperContent?.conclusion?.content || `
          <p className="text-base leading-relaxed text-gray-700 font-serif mb-6">
      ...
      {/* References */}
      <ParallaxSection 
        image={whitepaperContent?.references?.heroImageUrl || referencesImg} 
        title={whitepaperContent?.references?.title || "References"} 
        id="references" 
      />

      <ContentSection>
        {whitepaperContent?.references?.items?.length > 0 ? (
          <ul className="space-y-4 text-sm text-gray-900/80 font-serif">
            {whitepaperContent.references.items.map((ref: any, index: number) => (
              <li key={index} className="flex gap-2">
                <span>■</span>
                <span>
                  {ref.authors && `${ref.authers}. `}{ref.year && `(${ref.year}). `}{ref.title && <em>{ref.title}.</em>}{ref.source && ` ${ref.source}.`}
                  {ref.url && (
                    <>
                      {" "}
                      <a href={ref.url} className="text-primary underline break-all" target="_blank" rel="noopener noreferrer">
                        {ref.url}
                      </a>
                    </>
                  )}
                  {!ref.url && ref.text && ` ${ref.text}`}
                </span>
              </li>
            ))}
          </ul>
      ...
      <footer className="border-t border-gray-200 py-8 text-center text-sm text-gray-600">
        {whitepaperContent?.footer?.copyright || '© 2025 DigitalQatalyst. All rights reserved.'}
      </footer>
    </div>
  )
};

export default DynamicWhitepaperPage;
