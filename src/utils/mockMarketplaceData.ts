...
const getFirstHeroImageFromContent = (
  content: string | null,
  type: string,
): string | null => {
...
export const fetchKnowledgeHubItems = async () => {
  try {
    ...
    const mappedItems = mediaItems.data.map((item: any) => {
      ...
      else if (t === "report" || t === "research" || t === "research report")
        type = "Research Report";
      else if (t === "whitepaper" || t === "white paper") type = "White Paper";
      else if (t === "expert-interview" || t === "expert interview")
        type = "Expert Interview";
      else if (t === "prediction-analysis" || t === "prediction analysis")
        type = "Prediction Analysis";
      else if (t === "case-study" || t === "case study" || t === "case-studies")
        type = "Case Study";
      ...
      const mappedEntry: any = {
        ...
        mediaType: type,
        filterType:
          type === "Expert Interview" || type === "Prediction Analysis"
            ? type
            : `${type}s`,
        businessStage: item.businessStage || "All Stages",
        domain:
          item.domain ||
          getDomainFromTags(Array.isArray(item.tags) ? item.tags : []),
        category: item.category || null,
        format: item.format || getFormatFromType(type),
        popularity:
          item.popularity ||
          getPopularityFromDate(item.publishedAt || item.publishDate),
        provider: {
          ...
          description:
            ...
            "Leading digital transformation consultancy specializing in cognitive organizations.",
        },
        ...
        imageUrl:
          item.thumbnailUrl ||
          item.heroImage ||
          getFirstHeroImageFromContent(item.content, type) ||
          getDefaultImageForType(type),
        date: formatDate(
          item.publishedAt || item.publishDate || item.createdAt,
        ),
      ...
      if (item.slug) {
        ...
        if (type === "Blog") mappedEntry.blogUrl = `/blog/${slug}`;
          ...
          mappedEntry.articleUrl = `/dtmi/article/${slug}`;
        else if (type === "News") mappedEntry.newsUrl = `/news/${slug}`;
        else if (type === "Guide") mappedEntry.guideUrl = `/guides/${slug}`;
        else if (type === "Expert Interview")
          mappedEntry.expertInterviewUrl = `/expert-interviews/${slug}`;
        else if (type === "Prediction Analysis")
          mappedEntry.detailsUrl = `/marketplace/dtmi/prediction-analysis/${slug}`;
        else if (type === "Podcast")
          mappedEntry.detailsUrl = `/podcast/${slug}`;
      } else {
        // Fallback to ID if slug is missing
        if (type === "Blog") mappedEntry.blogUrl = `/blog/${item.id}`;
        else if (type === "Article")
          mappedEntry.articleUrl = `/dtmi/article/${item.id}`;
        else if (type === "News") mappedEntry.newsUrl = `/news/${item.id}`;
        else if (type === "Guide") mappedEntry.guideUrl = `/guides/${item.id}`;
        else if (type === "Expert Interview")
          mappedEntry.expertInterviewUrl = `/expert-interviews/${item.id}`;
        else if (type === "Prediction Analysis")
          mappedEntry.detailsUrl = `/marketplace/dtmi/prediction-analysis/${item.id}`;
        else if (type === "Podcast")
          mappedEntry.detailsUrl = `/podcast/${item.id}`;
      }
      ...
      if (type === "Video") {
        ...
        mappedEntry.downloadCount =
          (item as any).downloadCount || Math.floor(Math.random() * 5000) + 100;
...
const getProviderFromAuthor = (item: any): string => {
  ...
  if (typeof author === "string") {
    if (author.includes("Stéphane") || author.includes("Niango"))
      return "DigitalQatalyst";
    if (author.includes("Kaylynn") || author.includes("Océanne"))
      return "DigitalQatalyst";
  }
...
const getDefaultImageForType = (type: string): string => {
  const typeToImage: Record<string, string> = {
    ...
    Video:
      "https://images.unsplash.com/photo-1574717024653-61fd2cf4d44d?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80",
    Podcast:
      "https://images.unsplash.com/photo-1590602847861-f357a9332bbc?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80",
    Event:
...
const getAuthorInfo = (item: any) => {
  ...
  if (!authorName)
    return item.author && typeof item.author === "object"
      ? item.author
      : undefined;
...
export const getKnowledgeHubItems = async () => {
  try {
    ...
    const uniqueMockItems = mockKnowledgeHubItems.filter(
      (i) => !realIds.has(String(i.id)),
    );

    // Combine real database items with unique mock items (including books)
    ...
    // Fallback to mock data (including books) sorted by date
...
export const mockKnowledgeHubItems = [
  // Books from the Books Landing Page
  {
    id: "book-6xd-six-digital-perspectives",
    ...
    digital_perspective: "D1 - Digital Economy 4.0 (E4.0)",
    digital_stream: null,
    digital_domain: null,
    digital_sector: null,
    content_type: "Books",
    ...
    bookUrl: "/books/6xd-six-digital-perspectives/review",
  },
  {
    id: "book-digital-transformation-playbook",
    ...
    bookUrl: "/books/digital-transformation-playbook/review",
  },
  {
    id: "book-economics-of-data-analytics",
    ...
    author: {
      name: "Bill Schmarzo",
      title: "Data Economics Expert",
      avatar: "/images/default_avatar.png",
    },
    bookUrl: "/books/economics-of-data-analytics/review",
  },
  {
    id: "book-driving-digital-strategy",
    ...
    bookUrl: "/books/driving-digital-strategy/review",
  },
  {
    id: "book-demand-driven-business-strategy",
    title:
    ...
    businessStage: "All Stages",
    domain: "Strategy & Growth",
    category: "Market-Driven Strategy",
    format: "Downloadable Templates",
    popularity: "Featured",
    provider: {
      name: "DigitalQatalyst",
    ...
    bookUrl: "/books/demand-driven-business-strategy/review",
  },
  {
    id: "book-digital-transformation-subconscious-minds",
    title:
    ...
    digital_perspective: "D2 - Digital Cognitive Organisation (DCO)",
    digital_stream: null,
    digital_domain: null,
    digital_sector: null,
    content_type: "Books",
    ...
    author: {
      ...
      title: "Cognitive AI Expert",
      avatar: "/images/default_avatar.png",
    },
    bookUrl: "/books/digital-transformation-subconscious-minds/review",
  },
  {
    id: "book-integrating-business-ai",
    title:
    ...
    bookUrl: "/books/integrating-business-ai/review",
  },
  {
    id: "book-digital-hr-critical-management",
    title:
    ...
    readTime: "276 pages",
    author: {
      name: "Amelia Manuti, Pasquale Davide de Palma",
      title: "Digital HR Expert",
      avatar: "/images/default_avatar.png",
    },
    bookUrl: "/books/digital-hr-critical-management/review",
  },
  {
    id: "book-sizing-prize-ai-future",
    ...
    readTime: "334 pages",
    author: {
      name: "PwC Research Team",
      title: "AI Economics Researchers",
      avatar: "/images/default_avatar.png",
    },
    bookUrl: "/books/sizing-prize-ai-future/review",
  },
  {
    id: "book-machine-platform-crowd",
    ...
    digital_perspective: "D3 - Digital Business Platform (DBP)",
    digital_stream: null,
    digital_domain: null,
    digital_sector: null,
    content_type: "Books",
    ...
    imageUrl: "/DBP/imgi_106_61DnUeDO1aL._SL1057_.jpg",
    date: "February 20, 2025",
    readTime: "356 pages",
    author: {
      name: "Andrew McAfee, Erik Brynjolfsson",
      title: "Platform Economics Experts",
      avatar: "/images/default_avatar.png",
    },
    bookUrl: "/books/machine-platform-crowd/review",
  },
  {
    id: "book-digital-infrastructures-innovation",
    ...
    imageUrl: "/DBP/imgi_25_images.jpg",
    date: "May 10, 2025",
    readTime: "298 pages",
    author: {
      name: "Ray Hackney, et al.",
      title: "Digital Infrastructure Experts",
      avatar: "/images/default_avatar.png",
    },
    bookUrl: "/books/digital-infrastructures-innovation/review",
  },
  {
    id: "book-platform-revolution",
    ...
    imageUrl: "/DBP/imgi_41_default.jpg",
    date: "January 25, 2025",
    readTime: "334 pages",
    author: {
      name: "Geoffrey G. Parker, Marshall W. Van Alstyne, Sangeet Paul Choudary",
      title: "Platform Strategy Experts",
      avatar: "/images/default_avatar.png",
    },
    bookUrl: "/books/platform-revolution/review",
  },
  {
    id: "book-transform-14-behaviours",
    title:
    ...
    businessStage: "All Stages",
    domain: "Strategy & Growth",
    category: "Human-Centric Leadership",
    format: "Downloadable Templates",
    popularity: "Latest",
    provider: {
      name: "DigitalQatalyst",
    ...
    digital_perspective: "D5 - Digital Worker & Digital Workspace",
    digital_stream: null,
    digital_domain: null,
    digital_sector: null,
    content_type: "Books",
    ...
    readTime: "342 pages",
    author: {
      name: "Ian Murrin, Rajesh Jethwam, Mike Wright",
      title: "Digital Transformation Experts",
      avatar: "/images/default_avatar.png",
    },
    bookUrl: "/books/transform-14-behaviours/review",
  },
  {
    id: "book-co-intelligence-ai",
    ...
    author: {
      name: "Ethan Mollick",
      title: "AI Collaboration Expert",
      avatar: "/images/default_avatar.png",
    },
    bookUrl: "/books/co-intelligence-ai/review",
  },
  {
    id: "book-digital-transformation-siebel",
    title:
    ...
    bookUrl: "/books/digital-transformation-siebel/review",
  },
  {
    id: "book-cognitive-organizations-ai",
    title:
    ...
    bookUrl: "/books/cognitive-organizations-ai/review",
  },
  {
    id: "book-see-sooner-act-faster",
    title:
    ...
    readTime: "289 pages",
    author: {
      name: "George Day, Paul Schoemaker",
      title: "Digital Leadership Experts",
      avatar: "/images/default_avatar.png",
    },
    bookUrl: "/books/see-sooner-act-faster/review",
  },
  {
    id: "book-coming-wave-technology",
    title:
    ...
    author: {
      ...
      title: "Technology & Society Expert",
      avatar: "/images/default_avatar.png",
    },
    bookUrl: "/books/coming-wave-technology/review",
  },
  // Existing marketplace items continue below...
  {
    id: "cognitive-enterprises-prediction",
    title:
      "The Next Decade in Digital Transformation: How Cognitive Enterprises Will Drive Global Economic Growth",
    description:
      "By 2030, the rise of Digital Cognitive Organizations (DCOs) will fundamentally reshape the global economy, driving growth through AI-powered automation, cross-sector collaboration, and data-driven decision-making.",
    ...
    provider: {
      ...
      description:
        "Digital Transformation Management Insights - Leading research in enterprise AI and cognitive systems.",
    },
    tags: [
  ...
  },
  {
    ...
    description:
      "An exclusive session on the latest digital transformation strategies for modern businesses.",
    ...
    imageUrl:
      "https://images.unsplash.com/photo-1557804506-669a67965ba0?ixlib=rb-4.0.3&auto=format&fit=crop&w=1274&q=80",
  ...
  },
  {
    ...
    title:
      "Expert Interview: Digital Transformation Strategies for Modern Businesses",
    description:
  ...
  },
  {
    ...
    title:
      "The Rise of Digital Cognitive Organizations: 2025-2030 Outlook Analysis",
    description:
  ...
  },
  {
    ...
    title:
      "From 2025 to 2030: How AI-Driven Decision Making Will Shape Corporate Strategy and Governance",
    description:
  ...
  },
  {
    ...
    imageUrl: "/images/Blog 01_hero.png",
  ...
  },
  {
    ...
    imageUrl: "/images/Blog 02_hero.png",
  ...
  },
  {
    ...
    imageUrl: "/images/Blog 03_hero.png",
    date: "December 10, 2025",
    readTime: "7 min read",
  ...
  },
  {
    ...
    imageUrl: "/images/Blog 04_hero.png",
  ...
  },
  {
    ...
    imageUrl: "/images/Blog 05_hero.png",
    date: "December 5, 2025",
    readTime: "10 min read",
  ...
  },
  {
    ...
    imageUrl: "/images/Blog 06_hero.png",
  ...
  },
  {
    ...
    imageUrl: "/images/Blog 07_hero.png",
  ...
  },
  {
    ...
    imageUrl: "/images/Blog 08_hero.png",
  ...
  },
  {
    id: "23",
    title:
      "The End of Multitasking? How Automation Will Free Humans for High-Value Cognitive Work",
    description:
    ...
    imageUrl: "/images/Blog 09_hero.png",
  ...
  },
  {
    ...
    description:
      "This comprehensive whitepaper explores the evolving landscape of digital transformation, providing actionable insights and strategic frameworks for organizations looking to thrive in the digital economy.",
  ...
  },
  {
    ...
    description:
      "Join our expert panel as they discuss the latest trends in artificial intelligence and how businesses can leverage AI for digital transformation. This episode covers practical implementation strategies, common challenges, and success stories from leading organizations.",
    mediaType: "Podcast",
    filterType: "Podcasts",
    businessStage: "All Stages",
    domain: "Technology & Innovation",
...
export const mockKnowledgeHubFilterOptions = {
  ...
  businessStages: [{ id: "all", name: "All Stages" }],
  domains: [
    { id: "strategy", name: "Strategy & Growth" },
    { id: "technology", name: "Technology & Innovation" },
  ],
  formats: [{ id: "indepth", name: "In-Depth Reports" }],
  popularity: [
...
export const mockFinancialServices = [
  {
    id: "fin-1",
    title:
      "SME Growth Loan: Low-Interest Financing for Business Expansion in Abu Dhabi",
    description:
  ...
  },
  {
    id: "fin-2",
    title:
      "Startup Capital Fund: Equity Financing for Technology and Healthcare Innovators",
    description:
  ...
  },
  {
    id: "fin-3",
    title:
      "Business Insurance Package: Comprehensive Coverage for Abu Dhabi Enterprises",
    description:
  ...
  },
  {
    id: "fin-4",
    title:
      "Export Credit Guarantee: Financial Protection for International Trade Activities",
    description:
  ...
  },
  {
    id: "fin-5",
    title:
      "Business Credit Card: Specialized Solutions with Rewards for Business Expenses",
    description:
  ...
  },
  {
    id: "fin-6",
    title:
      "Equipment Financing: Flexible Solutions for Business Machinery and Equipment",
    description:
...
export const mockExpertInterviews: ExpertInterview[] = [
  {
    ...
    title:
      "From Transformation to Cognition: Why the Future Belongs to Digital Cognitive Organizations",
    description:
      "An in-depth conversation with Dr. Stéphane Niango about the evolution from traditional digital transformation to cognitive organizations and what this means for the future of business.",
    content: {
      introduction:
        "In this comprehensive interview, we explore the evolving landscape of digital transformation with Dr. Stéphane Niango, a leading expert in Digital Cognitive Organizations. As businesses worldwide grapple with the rapid pace of technological change, Dr. Niango offers insights into why traditional approaches to digital transformation are no longer sufficient and how organizations must evolve to become truly cognitive entities.",
      sections: [
        {
          id: "q1",
          question:
            "Dr. Niango, you've been advocating for a shift from traditional digital transformation to what you call 'Digital Cognitive Organizations.' Can you explain what this means?",
          answer:
            "Traditional digital transformation has been about adopting new technologies to improve existing processes. But we're now entering what I call Economy 4.0, where the fundamental nature of business itself is changing. Digital Cognitive Organizations, or DCOs, are entities that can think, learn, and adapt in real-time. They don't just use AI as a tool—they become inherently intelligent systems that can process information, make decisions, and evolve continuously. Think of it as the difference between a calculator and a brain. A calculator performs operations when instructed, but a brain processes, learns, and adapts autonomously.",
        },
        {
          id: "q2",
          question:
            "What are the key characteristics that distinguish a Digital Cognitive Organization from a traditionally digitized company?",
          answer:
    ...
    expert: {
      ...
      socialLinks: [
        {
          platform: "linkedin",
          url: "https://linkedin.com/in/stephane-niango",
        },
    ...
    seo: {
      metaTitle:
        "Expert Interview: Digital Cognitive Organizations with Dr. Stéphane Niango",
      metaDescription:
        "Discover the future of business with Dr. Stéphane Niango as he explains how Digital Cognitive Organizations are revolutionizing the way companies operate in Economy 4.0.",
      keywords: [
        "digital transformation",
        "cognitive organizations",
        "AI strategy",
...
export const getExpertInterviewBySlug = (
  slug: string,
): ExpertInterview | undefined => {
...
export const getRelatedContentForInterview = (interviewId: string) => {
  ...
  const relatedArticles = mockKnowledgeHubItems.filter(
    (item) =>
